...
FluidSystem::FluidSystem()
{
    ...
    f_PointDistance = 0.01;  //粒子半径

    materal = new Materal[1];
    materalCapcitySize = 1;
...
void FluidSystem::_addFluidVolume(const Box& fluidBox, float spacing, glm::vec3 velocity, short materalIndex)
{
    //int i = 0;
    //按照步长为spacing进行空间遍历；
    for (float z = fluidBox.max.z; z >= fluidBox.min.z; z -= spacing)
        for (float y = fluidBox.min.y; y <= fluidBox.max.y; y += spacing)
            for (float x = fluidBox.min.x; x <= fluidBox.max.x; x += spacing)
            {
                //printf("-i: %d\n", i);
                //添加点；
                Point* p = f_PointBuffer.addPointReuse(materalIndex);
                //初始化点坐标；
                p->pos = glm::vec3(x, y, z);
                p->velocity = velocity;
                //p->materalIndex = materalIndex;
                //i++;
...
bool FluidSystem::addPoint(glm::vec3 min, glm::vec3 max, glm::vec3 originVelocity)
{
    ...
    _addFluidVolume(Box(min, max), f_PointDistance / f_unitScale, originVelocity, 0);
    posData.clear();
    posData = std::vector<GLfloat>(6 * f_PointBuffer.getPointNumber());
    return true;
}

bool FluidSystem::addPoint(glm::vec3 min, glm::vec3 max, glm::vec3 originVelocity, MateralIndex materalIndex)
{
    if (f_PointBuffer.getPointNumber() >= ELEM_MAX || materalIndex >= materalCapcitySize)
        return false;
    _addFluidVolume(Box(min, max), f_PointDistance / f_unitScale, originVelocity, materalIndex);
    posData.clear();
    posData = std::vector<GLfloat>(6 * f_PointBuffer.getPointNumber());
    printf("size:%d\n", posData.size());
    return true;
...
bool FluidSystem::addPointTick(glm::vec3 min, glm::vec3 max, glm::vec3 originVelocity)
{
    if (f_PointBuffer.getPointNumber() >= ELEM_MAX)
        return false;
    if (count_tick++ >= tick_)
...
bool FluidSystem::addPointTick(glm::vec3 min, glm::vec3 max, glm::vec3 originVelocity, MateralIndex materalIndex)
{
    if (f_PointBuffer.getPointNumber() >= ELEM_MAX || materalIndex >= materalCapcitySize)
        return false;
    if (count_tick++ >= tick_)
...
MateralIndex FluidSystem::addMateral(Materal* newMateral)
{
    Materal* NEWM = new Materal[materalCapcitySize + 1];
    memcpy(NEWM, materal, sizeof(Materal) * materalCapcitySize);
    memcpy(&(NEWM[materalCapcitySize]), newMateral, sizeof(Materal));
    materalCapcitySize++;
    delete[] newMateral;
    delete[] materal;
    materal = NEWM;
    printf("size %d\n", materalCapcitySize);
    for (int i=0;i<materalCapcitySize;i++)
    {
        printf("%f\n", materal[i].color.g);
    }
    return materalCapcitySize - 1;
}
...
void FluidSystem::_init(unsigned short maxPointCounts, const Box& wallBox, const Box& initFluidBox, const glm::vec3& gravity)
{
    ...
    //计算粒子间距
    //(质量/密度)^(1/3)=粒子间距；
    f_PointDistance = pow(materal[0].f_PointMass / materal[0].f_restDensity, 1.0 / 3.0);
    //在initFluidBox空间中构造初始流体；
    _addFluidVolume(initFluidBox, f_PointDistance / f_unitScale);
    //整个粒子缓冲，用于快速绑定VBO；
    posData = std::vector<GLfloat>(6 * f_PointBuffer.getPointNumber(), 0);
}
...
void FluidSystem::_computerPressure()
{
        ...
        for (int cell = 0; cell < 8; cell++)
        {
            ...
            while (pndx != -1) {
                ...
                if (pj == pi)
                    ...
                    sum += pow(smoothR2, 3.0) * materal[pj->materalIndex].f_PointMass;
                else
                {
                    // 计算实际的向量；
                    ...
                    if (smoothR2 > r2)
                    {
                        ...
                        //sum += pow(h2_r2, 3.0);
                        sum += pow(h2_r2, 3.0) * materal[pj->materalIndex].f_PointMass;
                        // 该点的的邻接表满了，无法插入更多节点；
    ...
    FINALL:
        ...
        //完成计算；
        //pi->density = f_kernelPoly6 * materal[pi->materalIndex].f_PointMass * sum;
        pi->density = f_kernelPoly6 * sum;
        //计算压强：
        pi->pressure = (pi->density - materal[pi->materalIndex].f_restDensity) * f_gasConstantK;
    }
}

int max = 0;
#define thread_max 26

void FluidSystem::_computerForce(int i_)
{
    int begin = int(float(i_) / float(thread_max) * max);
    int end = int(float(i_ + 1) / float(thread_max) * max);
    ...
    for (unsigned int i = begin; i < end; i++)
    {
        ...
        for (unsigned int j = 0; j < neighborCounts; j++)
        {
            ...
            pressureS += ri_rj * pterm / distanceR * materal[pj->materalIndex].f_PointMass;
            //仅被使用一次，因此写成宏；
            //粘度计算公式；
            #define vterm (h_r / (pj->density))
            //viscosityS += (pj->velocityEval - pi->velocityEval) * vterm;
            viscosityS += (pj->velocityEval - pi->velocityEval) * vterm * materal[pj->materalIndex].f_PointMass;
        }
        //float PointMass = materal[pi->materalIndex].f_PointMass;
        //pressure中提取出来的公有项：
        //#define pressureH (-f_kernelSpiky * PointMass / (2.0f * pi->density))
        #define pressureH (-f_kernelSpiky / (2.0f * pi->density))
        //viscosity中提取出来的公有项：
        //#define viscosityH (f_kernelViscosity * materal[pi->materalIndex].f_viscosity * PointMass / (pi->density))
        #define viscosityH (f_kernelViscosity * materal[pi->materalIndex].f_viscosity / (pi->density))
        //计算最后的加速度：
...
void FluidSystem::_advance(int i_) {
    int begin = int(float(i_) / float(thread_max) * max);
    int end = int(float(i_+1) / float(thread_max) * max);
    //设置时间间隔；
    ...
    for (unsigned int i = begin; i < end; i++)
    {
        ...
        //弹入位置数据，用于快速绑定VBO；
        short index = 3 * i * 2;
        //printf("[%3d] here %5d  %4f %4f %4f\n", i_, i, p->pos.x,p->pos.y,p->pos.z);
        //(glm::vec3)(posData[index]) = p->pos;
        posData[index] = p->pos.x;
        posData[index + 1] = p->pos.y;
        posData[index + 2] = p->pos.z;
        //printf("[%3d] here %d\n", i_, i);
        //(glm::vec3)(posData[index + 3]) = materal[p->materalIndex].color;
        //(glm::vec3)(posData[index+3]) = p->pos;
        glm::vec3 color = materal[p->materalIndex].color;
        //glm::vec3 color = p->pos;
        posData[index + 3] = color.r;
        posData[index + 4] = color.g;
        posData[index + 5] = color.b;
    }
}
